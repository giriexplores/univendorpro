...
export function ShoppingCart({
  ...
  const handleUpdateQuantity = (productId: number, quantity: number) => {
    ...
    // if (isLoggedIn) {
    // updateCartMutation.mutate({ productId, quantity }); // <-- Your TRPC mutation
    // } else {
  ...
  const handleDeleteItem = (productId: number) => {
    // if (isLoggedIn) {
    //   removeFromCartMutation.mutate(productId); // <-- Your TRPC mutation
    // } else {
                ...
                {cartItems.map((item, index) => (
                  ...
                  >
                    <div className="flex flex-col items-start justify-center w-16">
                      <img
                        src={
                          item.product?.selectedColor?.imageUrl || item.product?.imageUrl ||
                      ...
                      {item.product?.selectedColor && (
                        <span className="text-xs text-gray-500 mt-1">
                          Color:{" "}
                          {item.product?.selectedColor?.name || "Unknown Color"}
                        </span>
                      )}
                      {item.product?.selectedSize && (
                        <span className="text-xs text-gray-500 mt-1">
                          Size:{" "}
                          {item.product?.selectedSize || "Unknown Size"}
                        </span>
                      )}
                    </div>
